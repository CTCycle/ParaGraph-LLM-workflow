...
describe('nodeCategory schema', () => {
    it('uses prompt, memory, retrieval, and text segmentation categories with no fragmentation fallback', () => {
        expect(NODE_CATEGORY_ORDER).toContain('prompt')
        expect(NODE_CATEGORY_ORDER).toContain('memory')
        ...
        expect(NODE_CATEGORY_LABELS.memory).toBe('Memory')
        ...
        expect(toNodeCategoryFilter('memory')).toBe('memory')
