...
class ConfigurationRepository:
    ...
    def _serialize_profile_summary(self, profile_row: ConfigurationProfile) -> dict[str, str]:
        return {
    ...
    def list_configuration_profiles(self, session_name: str | None = None) -> dict[str, Any]:
        normalized_session_name = self._normalize_session_name(session_name)
        with Session(database.backend.engine) as db_session:
            session_row, created = self._get_or_create_session(db_session, normalized_session_name)
            if created:
            ...
            profile_rows = list(
                db_session.execute(
                    ...
                    .order_by(ConfigurationProfile.updated_at.desc(), ConfigurationProfile.profile_name.asc())
                ).scalars()
            )
            return {
                'session_name': session_row.session_name,
                'profiles': [self._serialize_profile_summary(row) for row in profile_rows],
            }

    def load_configuration_profile(self, *, session_name: str | None, profile_name: str) -> dict[str, Any]:
        normalized_session_name = self._normalize_session_name(session_name)
        ...
        with Session(database.backend.engine) as db_session:
            ...
            if profile_row is None:
                raise KeyError(f"Configuration profile '{normalized_profile_name}' was not found")

            payload = profile_row.configuration_json
            if not isinstance(payload, dict):
                raise ValueError(f"Configuration profile '{normalized_profile_name}' is invalid")
