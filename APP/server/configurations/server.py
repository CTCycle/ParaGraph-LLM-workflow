...
from APP.server.common.utils.types import (
    coerce_bool,
    coerce_float,
    coerce_int,
    coerce_str_or_none,
)
from APP.server.configurations.base import ensure_mapping, load_configuration_data
from APP.server.entities.settings import (
    DatabaseSettings,
    GlobalSettings,
    JobsSettings,
    ServerSettings,
...
def build_global_settings(data: dict[str, Any]) -> GlobalSettings:
    ...
    return GlobalSettings(seed=coerce_int(payload.get("seed"), 42))
...
def build_database_settings(payload: dict[str, Any] | Any) -> DatabaseSettings:
    ...
    insert_batch_size = coerce_int(os.getenv("DB_INSERT_BATCH_SIZE"), 1000, minimum=1)
    ...
    return DatabaseSettings(
        ...
        port=coerce_int(os.getenv("DB_PORT"), 5432, minimum=1, maximum=65535),
        ...
        connect_timeout=coerce_int(os.getenv("DB_CONNECT_TIMEOUT"), 10, minimum=1),
        insert_batch_size=insert_batch_size,
    )
...
def build_server_settings(data: dict[str, Any] | Any) -> ServerSettings:
    ...
    return ServerSettings(
        ...
        global_settings=build_global_settings(payload.get("global")),
