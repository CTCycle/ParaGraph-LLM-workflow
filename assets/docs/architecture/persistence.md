...
- Alembic revisions live under `app/server/migrations`, and `alembic_version` records the synchronized revision. Empty databases are created by `upgrade head`; populated pre-Alembic databases are adopted only after their schema matches the supported baseline. Incompatible or partial schemas fail closed without destructive repair.
...
Review and edit every autogenerated revision before applying it; name new constraints and indexes explicitly. Do not rebuild populated baseline tables solely to retrofit names. Use launcher option 4 for the supported application initialization path, including legacy-schema adoption and startup locking.
...
- The workflow editor persists its `execution_session_id` alongside the local
  ...
  and Chat node ID, using the configured in-memory, file, or database history
  repository.
