...
- Related structured nodes include `STRUCTURED_INPUT`, `STRUCTURED_OUTPUT`, `JSON_VALIDATE_REPAIR`, and `OUTPUT_PARSER`.
...
- Additional RAG nodes include:
  ...
  - `CITATION_FORMATTER`
  - `GROUNDING_CHECKER`
- OCR returns an `ocr_engine_unavailable` error when the host does not provide the `tesseract` executable.

## Text Processing And Advanced Text
Processing coverage includes normalization, regex extraction and replacement, token and semantic splitting, join and merge, deduplication, metadata attachment, language detection, token counting, truncation, summarization, rewriting, claim extraction, contradiction detection, entity extraction and resolution, PII detection and redaction, prompt injection detection, instruction stripping, diffing, patch application, table extraction, Markdown parsing, code block extraction, citation extraction, date normalization, unit and number normalization, classifier scaffolds, quality scoring, and compression.
...
- Control nodes include `IF_TEXT_CONTAINS`, `SWITCH_BY_LABEL`, `MAP_OVER_CHUNKS`, `REDUCE_CHUNKS`, `BATCH_PROCESSOR`, `CACHE_NODE`, `HUMAN_REVIEW_GATE`, `ERROR_FALLBACK`, and `TRACE_DEBUG_VIEWER`.
