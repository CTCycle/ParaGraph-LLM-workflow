...
| `PG-T0-03` | Launcher start/stop lifecycle | `PARTIAL` | [startup guidance](runtime/startup.md); on 2026-09-22, option 1 launched the backend (`/docs`) and Vite preview twice, the UI was inspected in the in-app browser, and option 2 stopped the ParaGraph processes with both configured ports clear. Conflict checks covered decline/approval, distinct PIDs, and one PID owning both ports. Fingerprint checks covered reuse, missing output/stamp, invalid stamp, changed Vite API value, source addition/content change, timestamp-only source change, and a `FASTAPI_PORT`-only change. Backend/frontend checks passed (373 backend tests; 46 frontend unit tests). | Still exercise launcher-PID refusal, process-exit and termination-failure/survival races, port-reoccupation race, and the remaining build-input/failed-build matrix; capture comparable before/after warm-start timings. |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `app.startup.local` | `WORKING` | Windows launcher, environment bootstrap, separate backend/frontend startup, configured ports, and process cleanup. | [Startup guidance](runtime/startup.md); [PG-T0-02](../QA/PG-T0-02-f43590f.md) records fresh/repeat option 3, option 5, and migration-preservation checks. 2026-09-22 Windows checks started option 1 twice, reached backend `/docs` and frontend preview, inspected the rendered UI, exercised port-conflict confirmation paths, then used option 2 and confirmed both ports were clear. PowerShell parsing, frontend fingerprint state checks, and current backend/frontend suites passed. | npm reports a non-failing Node engine mismatch for a dev dependency (`ISSUE-004`) and an untriaged audit summary (`ISSUE-005`). | — | 2026-09-22 | unit + E2E + manual | [Runtime configuration](runtime/configuration.md), [deployment](runtime/deployment.md) | Complete the remaining PG-T0-03 launcher matrix and comparable warm-start timing checks. |
...
| `app.startup.local` | Working | Remaining PG-T0-03 option 1/2 failure-and-race cases and comparable warm-start timings. | Medium |
