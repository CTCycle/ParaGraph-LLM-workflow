...
| `PG-T0-03` | Launcher start/stop lifecycle | `PARTIAL` | [safety-race report](../QA/PG-T0-03-3de7520.md) records five passing deterministic checks; the live approved-conflict attempt failed closed when Windows denied `taskkill` for a task-owned listener. Earlier 2026-09-22 evidence in [startup guidance](runtime/startup.md) recorded option 1 launch, UI inspection, option 2 cleanup, conflict confirmation paths, fingerprint cases, and passing backend/frontend suites. | Re-run approved termination and option 1/2 lifecycle where `taskkill` can stop a task-owned process; finish the remaining build-input/failed-build matrix and comparable warm-start timings. |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `app.startup.local` | `WORKING` | Windows launcher, environment bootstrap, separate backend/frontend startup, configured ports, and process cleanup. | [Startup guidance](runtime/startup.md); [PG-T0-02](../QA/PG-T0-02-f43590f.md) records fresh/repeat option 3, option 5, and migration-preservation checks. 2026-09-22 Windows checks started option 1 twice, reached backend `/docs` and frontend preview, inspected the rendered UI, exercised port-conflict confirmation paths, then used option 2 and confirmed both ports were clear. [PG-T0-03](../QA/PG-T0-03-3de7520.md) adds five passing focused race checks; the 2026-09-23 approved-conflict run was blocked when Windows denied `taskkill` for the task-owned listener, and the launcher aborted before app startup. | npm reports a non-failing Node engine mismatch for a dev dependency (`ISSUE-004`) and an untriaged audit summary (`ISSUE-005`). | — | 2026-09-23 (focused race slice) | unit + E2E + manual | [Runtime configuration](runtime/configuration.md), [deployment](runtime/deployment.md) | Re-run approved termination and option 1/2 shutdown where `taskkill` can stop a task-owned process; finish the remaining PG-T0-03 build failure/input cases and warm-start timings. |
...
| `app.startup.local` | Working | Successful approved termination and option 1/2 lifecycle remain unverified in this environment; the PG-T0-03 build failure/input matrix and comparable warm-start timings remain. | Medium |
