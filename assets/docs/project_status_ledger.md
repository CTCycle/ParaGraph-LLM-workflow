...
| --- | --- | --- | --- | --- |
| `PG-T0-01` | Clean CI backend/frontend baseline | `PASS` | [baseline and retest report](../QA/PG-T0-01-6a9e5e1.md); the passing GitHub Actions run is [35614890124](https://github.com/CTCycle/ParaGraph-LLM-workflow/actions/runs/35614890124) on `1b2a2b9920ce2ab9a28df2ecdf4b5070671b4e87`. | Continue with `PG-T0-02` and `PG-T0-03`; keep the five CI inputs explicit if the Vite contract changes. |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `app.startup.local` | `WORKING` | Windows launcher, environment bootstrap, separate backend/frontend startup, configured ports, and process cleanup. | [Startup guidance](runtime/startup.md); 2026-09-22 Windows checks started option 1 twice, reached backend `/docs` and frontend preview, inspected the rendered UI, exercised port-conflict confirmation paths, then used option 2 and confirmed both ports were clear. PowerShell parsing, frontend fingerprint state checks, and current backend/frontend suites passed. | The full conflict/build matrix, fresh option-3 bootstrap, and before/after timing comparison remain outstanding; details are recorded in the PG-T0-03 slice row above. | — | 2026-09-22 | unit + E2E + manual | [Runtime configuration](runtime/configuration.md), [deployment](runtime/deployment.md) | Complete the remaining PG-T0-03 manual matrix and a disposable option-3/bootstrap check. |
...
| `app.startup.local` | Working | Fresh option-3 bootstrap, remaining option 1/2 failure-and-race cases, and comparable warm-start timings. | Medium |
...
1. Complete `PG-T0-02` and `PG-T0-03` from disposable Windows/runtime boundaries.
