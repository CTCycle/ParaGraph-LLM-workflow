...
- An existing database from before Alembic is adopted only when its eight application tables match the supported baseline; partial or incompatible schemas stop startup with an actionable error and are never overwritten.
