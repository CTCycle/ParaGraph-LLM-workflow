...
###############################################################################
def _classifier(
    label: str, score: float, matches: list[Any], metadata: dict[str, Any] | None = None
) -> dict[str, Any]:
    return {
        "label": label,
        "score": score,
...
def _claim_extractor_executor(
    ...
    claims = [
        part.strip()
        for part in re.split(r"(?<=[.!?])\s+", _text(inputs))
        if part.strip()
    ]
    return {
        "result": [
...
def _contradiction_detector_executor(
    ...
    return _classifier(
        "contradiction" if negated or numeric_conflict else "neutral",
        1.0 if negated or numeric_conflict else 0.0,
        [],
    )
...
def _entity_extractor_executor(
    ...
    text = _text(inputs)
    entities = [
        {"text": match.group(0), "type": "proper_noun"}
...
def _entity_resolver_executor(
    ...
    values = [
        ...
        for item in (entities if isinstance(entities, list) else [entities])
    ]
    ...
    return {
        "result": [
            {"canonical": cluster[0], "aliases": cluster} for cluster in clusters
        ]
    }
...
def _pii_detector_executor(
    ...
    for kind, pattern in PII_PATTERNS.items():
        for match in re.finditer(pattern, text, re.IGNORECASE):
            matches.append(
                {"type": kind, "text": match.group(0), "span": list(match.span())}
            )
...
def _instruction_stripper_executor(
    ...
    lines = [
        ...
        if not re.search(
            r"ignore previous|system prompt|developer message|jailbreak", line, re.I
        )
    ]
    return {"result": "\n".join(lines)}
...
def _diff_text_executor(
    ...
    diff = difflib.unified_diff(
        _text(inputs, "before").splitlines(),
        _text(inputs, "after").splitlines(),
        lineterm="",
    )
    return {"result": "\n".join(diff)}
...
def _code_block_extractor_executor(
    ...
    _ = parameters
    blocks = [
        {"language": match.group(1), "code": match.group(2)}
...
def _citation_extractor_executor(
    ...
    return {
        "result": {
            "urls": urls,
            "footnotes": footnotes,
            "references": re.findall(r"\[[A-Za-z0-9 ,.-]+\]", text),
        }
    }
...
def _date_normalizer_executor(
    ...
    try:
        ...
        return {
            "result": {"iso": parsed.date().isoformat(), "datetime": parsed.isoformat()}
        }
...
ADVANCED_TEXT_HANDLERS = {
    ...
    "entity_resolver": NodeHandler(
        executor=_entity_resolver_executor, parameter_model=AdvancedTextParameters
    ),
