...
def _resolve_http_template_values(value: Any, variables: dict[str, Any]) -> Any:
    if isinstance(value, str):
        return Template(value).safe_substitute(
            {key: str(val) for key, val in variables.items()}
        )
    if isinstance(value, dict):
        return {
            key: _resolve_http_template_values(val, variables)
...
def _validate_http_url(url: str) -> None:
    ...
    allow_private = (
        os.getenv("PARAGRAPH_ALLOW_PRIVATE_HTTP_NODES", "").lower() == "true"
    )
...
def _build_headers(
    ...
    return {
        str(key): str(value)
        for key, value in _resolve_http_template_values(
            parameters.get("headers", {}), inputs
        ).items()
    }
...
def _execute_http_request(
    ...
    with httpx.Client(
        timeout=float(parameters.get("timeout_seconds", 30.0)), follow_redirects=False
    ) as client:
        response = client.request(
            ...
            json=_build_json_body(parameters, inputs)
...
HTTP_HANDLERS = {
    "http_get": NodeHandler(
        executor=_http_get_executor, parameter_model=HttpGetParameters
    ),
    "http_post": NodeHandler(
        executor=_http_post_executor, parameter_model=HttpPostParameters
    ),
    "http_put": NodeHandler(
        executor=_http_put_executor, parameter_model=HttpPutParameters
    ),
    "http_patch": NodeHandler(
        executor=_http_patch_executor, parameter_model=HttpPatchParameters
    ),
    "http_delete": NodeHandler(
        executor=_http_delete_executor, parameter_model=HttpDeleteParameters
    ),
}
