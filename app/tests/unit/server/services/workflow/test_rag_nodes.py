...
def test_html_to_text_strips_script_style_nav() -> None:
    result = _html_to_text_executor({}, {"html": "<script>x</script><nav>n</nav><p>Hello</p>"})
...
def test_context_builder_respects_token_budget() -> None:
    result = _context_builder_executor({"token_budget": 2}, {"chunks": [{"text": "one two three"}]})
    assert result["result"] == "one two"


def test_grounding_checker_marks_unsupported_claim_when_evidence_absent() -> None:
    result = _grounding_checker_executor({}, {"claim": "missing", "evidence": "present"})
