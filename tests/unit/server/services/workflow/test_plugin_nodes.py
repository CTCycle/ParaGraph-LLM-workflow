...
""".strip(),
    ...
    write_manifest(node_root / 'custom_script_node_v1.json', build_plugin_manifest('plugins/custom_script_node.py'))

    monkeypatch.setattr(node_module, 'NODE_ROOT', node_root)
    registry = node_module.NodeRegistry()

    result = registry.execute('CUSTOM_SCRIPT_NODE', 1, {'prefix': 'pre-'}, {'text': 'hello'})
...
def test_plugin_node_rejects_absolute_script_paths(monkeypatch, tmp_path: Path) -> None:
    ...
    write_manifest(node_root / 'custom_script_node_v1.json', build_plugin_manifest(str(absolute_script)))
