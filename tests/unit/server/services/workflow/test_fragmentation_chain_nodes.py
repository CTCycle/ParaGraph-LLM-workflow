...
def test_by_delimiter_chunks_loads_deferred_documents_from_file_paths(tmp_path: Path) -> None:
    source_dir = tmp_path / 'docs'
    source_dir.mkdir(parents=True, exist_ok=True)
    source = source_dir / 'deferred.txt'
    source.write_text('Deferred loading keeps text on disk. It is split later.', encoding='utf-8')
