...
def test_sqlite_repository_uses_resources_root_for_default_path(tmp_path: Path, monkeypatch) -> None:
...
def test_sqlite_repository_migrates_legacy_database_path(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setattr(sqlite_module, 'RESOURCES_PATH', str(tmp_path))
...
def test_sqlite_repository_save_load_and_count_rows(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setattr(sqlite_module, 'RESOURCES_PATH', str(tmp_path))
    repository = sqlite_module.SQLiteRepository(_build_settings())
...
def test_sqlite_repository_load_missing_table_returns_empty_frame(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setattr(sqlite_module, 'RESOURCES_PATH', str(tmp_path))
    repository = sqlite_module.SQLiteRepository(_build_settings())
...
def test_sqlite_repository_count_rows_raises_for_missing_table(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setattr(sqlite_module, 'RESOURCES_PATH', str(tmp_path))
    repository = sqlite_module.SQLiteRepository(_build_settings())
