...
def test_build_backend_initializes_sqlite_only_when_database_file_missing(
    ...
    monkeypatch.setattr(
        ...
        lambda _settings: SimpleNamespace(
            engine=object(), db_path=str(tmp_path / "database.db")
        ),
    )
    monkeypatch.setitem(
        backend_module.BACKEND_FACTORIES, "sqlite", backend_module.build_sqlite_backend
    )
...
def test_build_backend_never_initializes_postgres_on_startup(monkeypatch) -> None:
    ...
    monkeypatch.setitem(
        backend_module.BACKEND_FACTORIES,
        "postgres",
        backend_module.build_postgres_backend,
    )
    monkeypatch.setattr(
        backend_module,
        "get_server_settings",
